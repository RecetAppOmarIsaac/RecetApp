...
public class CategoriasService implements ICategoriasService {
	...
	public void modificarCategoria(CategoriaItem categoria) throws ServiceException {
		if (categoria == null) throw new IllegalArgumentException("Debe especificar una categor�a  para modificarla");
		try {
			Connection conn=DataBase.getConnection();
			...
			if (stmt.executeUpdate()==0) {
				throw new ServiceException("La categor�a con id "+categoria.getId()+" no existe en la base de datos");
			}
			stmt.close();
		} catch (SQLException e) {
			throw new ServiceException("No se ha podido modificar la categor�a "+ e);
		}
	}
	...
	public void eliminarCategoria(Long id) throws ServiceException {
		try {
			...
			stmt.setLong(1,id);
			if (stmt.executeUpdate()==0) {
				throw new ServiceException("La categor�a con id "+id+" no existe en la base de datos");
			}
			stmt.close();
		} catch (SQLException e) {
			throw new ServiceException("No se ha podido eliminar la categor�a "+ e);
		}
	...
	public CategoriaItem obtenerCategoria(Long id) throws ServiceException {
		...
		try {
			...
			ResultSet rs =stmt.executeQuery();
			if (!rs.next()) {
				throw new ServiceException("No existe ninguna categor�a con ese id ");
			} else {
				rs.previous();
				while(rs.next()){
